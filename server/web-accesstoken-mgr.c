...
is_one_time_operation (const char *op)
{
    return (strcmp(op, "view") == 0 ||
            strcmp(op, "download") == 0 ||
            strcmp(op, "download-dir") == 0 ||
            strcmp(op, "update") == 0);
}
...
static void
remove_token (SeafWebAccessTokenManager *mgr,
              const char *token,
              AccessInfo *info)
{
    GString *key = g_string_new (NULL);
    g_string_printf (key, "%s %s %s %s",
                     info->repo_id, info->obj_id, info->op, info->username);
                                        ...
                                        const char *token)
{
    ...
    if (info != NULL) {
        ...
        if (now - expire_time >= 0) {
            ...
            if (is_one_time_operation(info->op))
                remove_token (mgr, token, info);
