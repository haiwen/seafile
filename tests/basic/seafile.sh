...
while [ $# -ge 1 ]; do
  case $1 in
    ...
    "2" )
      ...
      bash -c "${ccnet_server} -c ${conf2} -D ${debug} -f -" &
      sleep 3
      bash -c "${seaf_server} -c ${conf2}  -d ${conf2}/seafile-data -D all -f -l -" &
      ;;
    "3" )
      ...
      ;;
    "4" )
      bash -c "${ccnet} -c ${conf4} -D ${debug} -f -" &
      sleep 3
      bash -c "${seaf_daemon} -c ${conf4} -d ${conf4}/seafile-data -w worktree/wt4 -D all -l -" &
      ;;
  esac
  shift
done
