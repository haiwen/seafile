...
'''This script would guide the seafile admin to setup seafile with PostgreSQL'''
...
class Utils(object):
    ...
    def welcome():
        ...
        welcome_msg = '''\
-----------------------------------------------------------------
This script will guide you to setup your seafile server using PostgreSQL.
...
-----------------------------------------------------------------''' % SERVER_MANUAL_HTTP
    ...
    def get_python_executable():
        ...
        variable which is set in setup-seafile-postgres.sh
...
class AbstractDBConfigurator(AbstractConfigurator):
    ...
    def __init__(self):
        ...
        self.seafile_postgres_user = ''
        self.seafile_postgres_password = ''
...
'''
    ...
    def ask_postgres_host_port(self):
        def validate(host):
            ...
            question = 'What is the port of postgres server?'
            key = 'postgres server port'
            default = '5432'
            ...
            # self.check_postgres_server(host, port)
            self.postgres_port = port

            return host

        question = 'What is the host of postgres server?'
        key = 'postgres server host'
        default = 'localhost'
        self.postgres_host = Utils.ask_question(question,
                                             key=key,
                                             default=default,
                                             validate=validate)

    def check_postgres_server(self, host, port):
        print '\nverifying postgres server running ... ',
        try:
            dummy = psycopg2.connect(host=host, port=port)
        except Exception:
            print
            raise InvalidAnswer('Failed to connect to postgres server at "%s:%s"' \
                                % (host, port))

        print 'done'

    def check_postgres_user(self, user, password):
        print '\nverifying password of user %s ... ' % user,
        tmpdb='postgres'
        if user != "postgres":
            tmpdb = "template1"
        kwargs = dict(host=self.postgres_host,
        ...
        try:
            ...
            if isinstance(e, psycopg2.OperationalError):
                raise InvalidAnswer('Failed to connect to postgres server using user "%s" and password "***": %s' \
                                    % (user, e.args[0]))
            else:
                raise InvalidAnswer('Failed to connect to postgres server using user "%s" and password "***": %s' \
                                    % (user, e))
    ...
    def create_seahub_admin(self):
        try:
            conn = psycopg2.connect(host=self.postgres_host,
                                   port=self.postgres_port,
                                   user=self.seafile_postgres_user,
                                   password=self.seafile_postgres_password,
                                   dbname=self.ccnet_db_name)
        except Exception, e:
            if isinstance(e, psycopg2.OperationalError):
                Utils.error('Failed to connect to postgres database %s: %s' % (self.ccnet_db_name, e.args[1]))
            else:
                Utils.error('Failed to connect to postgres database %s: %s' % (self.ccnet_db_name, e))

        cursor = conn.cursor()
        sql = '''\
CREATE TABLE IF NOT EXISTS EmailUser (id INTEGER NOT NULL PRIMARY KEY AUTO_INCREMENT, email VARCHAR(255), passwd CHAR(64), is_staff BOOL NOT NULL, is_active BOOL NOT NULL, ctime BIGINT, UNIQUE INDEX (email))'''
...
class NewDBConfigurator(AbstractDBConfigurator):
    '''Handles the case of creating new postgres databases for ccnet/seafile/seahub'''
    def __init__(self):
    ...
    def ask_questions(self):
        self.ask_postgres_host_port()

        self.ask_root_password()
        self.ask_seafile_postgres_user_password()

        self.ask_db_names()

    def generate(self):
        if not self.postgres_user_exists(self.seafile_postgres_user):
            self.create_user()
        self.create_databases()

    def ask_root_password(self):
        def validate(password):
            self.root_conn = self.check_postgres_user('postgres', password)
            return password

        question = 'What is the password of the postgres user?'
        key = 'root password'
    ...
    def postgres_user_exists(self, user):
        cursor = self.root_conn.cursor()

        sql = '''SELECT EXISTS(SELECT 1 FROM pg_roles WHERE rolname='%s') ''' % user
        ...
        try:
            ...
            if isinstance(e, psycopg2.OperationalError):
                Utils.error('Failed to check postgres user %s: %s' % (user, e.args[1]))
            else:
                Utils.error('Failed to check postgres user %s: %s' % (user, e))
        finally:
            cursor.close()


    def ask_seafile_postgres_user_password(self):
        def validate(user):
            if user == 'postgres':
                self.seafile_postgres_password = self.root_password
            else:
                question = 'Enter the password for DB user "%s":' % Utils.highlight(user)
                ...
                if self.postgres_user_exists(user):
                    self.check_postgres_user(user, password)
                self.seafile_postgres_password = password

            return user


        question = 'Enter the name for DB user of seafile. It will be created if it does not exist.'
        key = 'postgres user for seafile'
        default = 'seafile'
        self.seafile_postgres_user = Utils.ask_question(question,
                                                     key=key,
                                                     default=default,
                                                     validate=validate)
    ...
    def ask_db_names(self):
        self.ccnet_db_name = self.ask_db_name('ccnet_server', 'ccnet_db')
        self.seafile_db_name = self.ask_db_name('seafile_server', 'seafile_db')
    ...
    def create_user(self):
        self.root_conn.set_isolation_level(0)
        cursor = self.root_conn.cursor()
        sql = '''CREATE ROLE %s WITH PASSWORD '%s' LOGIN''' \
              % (self.seafile_postgres_user, self.seafile_postgres_password)
        try:
            cursor.execute(sql)
        except Exception, e:
            if isinstance(e, psycopg2.OperationalError):
                Utils.error('Failed to create postgres user %s: %s' % (self.seafile_postgres_user, e.args[1]))
            else:
                Utils.error('Failed to create postgres user %s: %s' % (self.seafile_postgres_user, e))
        finally:
            cursor.close()
    ...
    def create_db(self, db_name):
        self.root_conn.set_isolation_level(0)
        cursor = self.root_conn.cursor()
        sql = '''CREATE DATABASE %s WITH OWNER=%s ENCODING='utf8' ''' \
              % (db_name, self.seafile_postgres_user )
    ...
    def create_databases(self):
        ...
        if self.seafile_postgres_user != 'postgres':
...
class ExistingDBConfigurator(AbstractDBConfigurator):
    '''Handles the case of use existing postgres databases for ccnet/seafile/seahub'''
    def __init__(self):
        AbstractDBConfigurator.__init__(self)
    ...
    def ask_questions(self):
        ...
        self.ask_existing_postgres_user_password()
    ...
    def ask_existing_postgres_user_password(self):
        def validate(user):
            question = 'What is the password for postgres user "%s"?' % Utils.highlight(user)
            key = 'password for %s' % user
            password = Utils.ask_question(question, key=key, password=True)
            self.check_postgres_user(user, password)
            self.seafile_postgres_password = password
        ...
        question = 'Which postgres user to use for seafile?'
        key = 'postgres user for seafile'
        self.seafile_postgres_user = Utils.ask_question(question,
                                                     key=key,
                                                     validate=validate)
...
class CcnetConfigurator(AbstractConfigurator):
    ...
    def generate_db_conf(self):
        ...
        config.set(db_section, 'ENGINE', 'pgsql')
        config.set(db_section, 'HOST', db_config.postgres_host)
        config.set(db_section, 'PORT', db_config.postgres_port)
        config.set(db_section, 'USER', db_config.seafile_postgres_user)
        config.set(db_section, 'PASSWD', db_config.seafile_postgres_password)
...
class SeafileConfigurator(AbstractConfigurator):
    ...
    def generate_db_conf(self):
        ...
        if not config.has_section(db_section):
            config.add_section(db_section)
        config.set(db_section, 'type', 'pgsql')
        config.set(db_section, 'host', db_config.postgres_host)
        config.set(db_section, 'port', db_config.postgres_port)
        config.set(db_section, 'user', db_config.seafile_postgres_user)
        config.set(db_section, 'password', db_config.seafile_postgres_password)
...
'''
        text = template % dict(name=db_config.seahub_db_name,
                               username=db_config.seafile_postgres_user,
                               password=db_config.seafile_postgres_password,
    ...
    def do_syncdb(self):
        ...
        try:
            conn = psycopg2.connect(host=db_config.postgres_host,
                                   port=db_config.postgres_port,
                                   user=db_config.seafile_postgres_user,
                                   password=db_config.seafile_postgres_password,
                                   dbname=db_config.seahub_db_name)
        except Exception, e:
            if isinstance(e, psycopg2.OperationalError):
                Utils.error('Failed to connect to postgres database %s: %s' % (db_config.seahub_db_name, e.args[1]))
            else:
                Utils.error('Failed to connect to postgres database %s: %s' % (db_config.seahub_db_name, e))

#        conn.set_isolation_level(ISOLATION_LEVEL_AUTOCOMMIT)
        cursor = conn.cursor()

        sql_file = os.path.join(env_mgr.install_path, 'seahub', 'sql', 'postgres.sql')
...
'''
    config = {
        ...
        'db_user':              db_config.seafile_postgres_user
    }
...
def main():
    ...
    Utils.welcome()
# It seams psycopg2.Warning is actually an exeption mechanism while mysql.Warning is just a warning and not excepition being raised
